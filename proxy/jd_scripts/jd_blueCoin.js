    ...
    if (cookie) {
      ...
      try {
        if (`${coinToBeans}` !== "0") {
          ...
          console.log(
            "查询到您设置的是不兑换京豆选项，现在为您跳过兑换京豆。如需兑换，请去BoxJs设置或者修改脚本coinToBeans或设置环境变量MARKET_COIN_TO_BEANS\n"
          );
        }
        await msgShow();
      } catch (e) {
  ...
  if (prizeList && prizeList.length) {
    if (`${coinToBeans}` === "1000") {
      if (prizeList[0] && prizeList[0].type === 3) {
        console.log(
          `查询换${prizeList[0].name}ID成功，ID:${prizeList[0].prizeId}`
        );
      ...
      if (
        prizeList[0] &&
        prizeList[0].limit === prizeList[0] &&
        prizeList[0].finished
      ) {
      ...
      if (prizeList[1] && prizeList[1].type === 3) {
        console.log(
          `查询换${prizeList[1].name}ID成功，ID:${prizeList[1].prizeId}`
        );
      ...
      if (
        (prizeList[0] && prizeList[0].limit) ===
        (prizeList[0] && prizeList[0].finished)
      ) {
        $.beanerr = `${prizeList[0].name}`;
        return;
      }
      ...
      if (prizeId) {
        ...
        if (
          prizeList[i].targetNum &&
          prizeList[i].targetNum === prizeList[i].finishNum
        ) {
        ...
        console.log(
          `奖品兑换列表【${coinToBeans}】已下架，请检查活动页面是否存在此商品，如存在请检查您的输入是否正确`
        );
...
//换京豆
function smtg_obtainPrize(
  prizeId,
  timeout = 0,
  functionId = "smt_exchangePrize"
) {
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      let url = {
        url: `${JD_API_HOST}&functionId=${functionId}&clientVersion=8.0.0&client=m&body=${encodeURIComponent(
          JSON.stringify(body)
        )}&t=${Date.now()}`,
        headers: {
          Origin: `https://jdsupermarket.jd.com`,
          ...
          if (safeGet(data)) {
            ...
            if ($.data.data.bizCode === 0) {
              if (`${coinToBeans}` === "1000") {
                $.beanscount++;
                console.log(
                  `【京东账号${$.index}】${$.nickName || $.UserName} 第${
                ...
                if (
                  $.data.data.result.count === 20 ||
                  $.beanscount === coinToBeans ||
                  $.data.data.result.blue < $.blueCost
                )
                  return;
              } else {
                $.beanscount++;
