...
async function PrizeIndex() {
  ...
  await Promise.all([smtg_queryPrize(), smtg_materialPrizeIndex()]);
  ...
  if (prizeList && prizeList.length) {
    if (`${coinToBeans}` === "1000") {
      ...
      if (
        prizeList[1] &&
        prizeList[1].targetNum === prizeList[1] &&
        prizeList[1].finishNum
      ) {
      ...
      if (
        (prizeList[0] && prizeList[0].targetNum) ===
        (prizeList[0] && prizeList[0].finishNum)
      ) {
