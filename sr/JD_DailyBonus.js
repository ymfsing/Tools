...
var Key =
  "shshshfp=2428bd038bfc4b1be60435aa058ceea7; shshshfpa=ac00ec5a-7b77-faf8-e679-cce908912b80-1624417333; shshshsID=8b3b2573446fd0923fb310a74a4f19e3_1_1624417334027; shshshfpb=rnVq48avqC425tHgXjzde+g==; jcap_dvzw_fp=YSGtH-psR3K5vFUxyzCKwXaZu8o3LS6N1_gyFv5FwfizuUho-GaiqWOEZ9RQ2e7HzFzidQ==; TrackerID=bSfYEumJ_V5gKhS1kV8X6DYbN25PQrHpG6_m3AxCCr0To0O6gsq2L3OueZ7GX6XVChNSd4FzmqbVCvezJI1JK18do5AIXyXTaJrXl4tyf9B54YrBGdPETxVLkIYsW2bt; pt_key=AAJg0qRvADDv7XzJByrMnpJaQ-HVCWGxs_-5mgEDi-ajim_LEX0-LrD0PZ-Cu1RofaKb2T6v4gM; pt_pin=口九; pt_token=o7k7el45; pwdt_id=口九; sfstoken=tk01mcd031cd6a8sMiszeDMrMXgxaJT49KUo8pFcQyuu2PFl4R5E/CwxapMBcbbFPQP5ejEkWbk5lLrpLHjx1DXxrI1l; whwswswws=; mobilev=html5"; //单引号内自行填写您抓取的Cookie
    ...
    try {
      ...
      const numFix =
        !Key && !DualKey
          ...
          : Key && DualKey
          ? DualAccount
          : DualAccount - 1 || DualAccount;
      const Name =
        DualKey || OtherKey
          ? `【签到号${cnNum[numFix] || numFix}】:  ${DName}\n`
          : ``;
      ...
      if ($nobyda.isJSBox) {
        Shortcut =
          (typeof Shortcut == "undefined" ? "" : Shortcut) +
          Name +
          one +
          two +
          three +
...
function ReadCookie() {
  ...
  if (DeleteCookie) {
    if (
      $nobyda.read(EnvInfo) ||
      $nobyda.read(EnvInfo2) ||
      ($nobyda.read(EnvInfo3) || "[]") != "[]"
    ) {
...
async function JDUserSign2(s, key, title, tid) {
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      const JDUrl = {
        url:
          "https://jdjoy.jd.com/api/turncard/channel/sign?invokeKey=NRp8OPxZMFXmGkaE",
        headers: {
          Cookie: KEY,
        },
            ...
            if (cc.status && cc.data && cc.data.firstLoginStatus) {
              ...
              const qt =
                cc.status && cc.data && cc.data.firstLoginStatus === false
                  ...
                  ? cc.error.msg
                  : `原因: 未知`;
              merge.JDBuyCar.notify = `京东商城-汽车: 失败, ${qt}${
...
function GetCookie() {
  try {
    if (
      ...
      $request.headers &&
      $request.url !== "http://www.apple.com/"
    ) {
      let acObj = {};
      // 提取ck数据
      let CV = (
      ...
      let ckItems = CV.split(";")
        .filter((s) => /^(pt_key|pt_pin)=.+/.test(s))
        .sort();
      if (ckItems.length == 2) {
        acObj.cookie = ckItems.join(";") + ";";
        acObj.userName = decodeURIComponent(
          acObj.cookie.match(/pt_pin=(.+?);/)[1]
        );
      }
      ...
      if (!acObj.cookie) {
        ...
        try {
          ...
          console.log(
            `写入京东Cookie时转换京东扩展账号数据CookiesJD异常，扩展账号信息：\n${ocks}`
          );
          oldCks = [];
        }
        oldCks.forEach((item) => allCk.push(item.cookie));
        let [status, seatNo] = chooseSeatNo(
          acObj.cookie,
          allCk,
          /pt_pin=(.+?);/
        );
        if (status) {
          if (status > 0) {
            ...
            $nobyda.notify(
              `用户名: ${acObj.userName}`,
              ``,
              `${status == 2 ? `更新` : `写入`}京东 [账号${seatNo + 1}] Cookie${
                WT ? `成功 🎉` : `失败 ‼️`
              }`
            );
          } else {
